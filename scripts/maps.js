...
if(window.sessionStorage.map_tags) tags = window.sessionStorage.map_tags.split(','); else tags = allTags.slice(0);
...
onLoad = _ => {
	getElemList('link')[1].style.borderBottom = '3px solid ' + window.getComputedStyle(document.body, null).getPropertyValue('background-color');
	getElemList('link')[1].style.background = window.getComputedStyle(document.body, null).getPropertyValue('background-color');
};
...
function getCheckedTagAndRefresh(){
	...
	window.sessionStorage.map_tags = '';
	for(let i=0; i<getElemList('tag').length; i++){
		if(!getElemList('tag')[i].checked) continue;
		window.sessionStorage.map_tags += allTags[i];
		if(i < getElemList('tag').length-1) window.sessionStorage.map_tags += ',';
	}
	location.reload();
}
...
(async()=>{
	let qCol = await getDocs(query(collection(db, "projects"), where("type", "==", "map")));
